...
//ZTEBSP DangXiao 20120717,re-init i2c-4 controller when unexpected irq is triggered,start
static int tegra_i2c_init(struct tegra_i2c_dev *i2c_dev,int in_isr)
//ZTEBSP DangXiao 20120717,re-init i2c-4 controller when unexpected irq is triggered,end
{
	u32 val;
	int err = 0;
...
//ZTEBSP DangXiao 20120717,re-init i2c-4 controller when unexpected irq is triggered,start
	if(!in_isr)
	   ...
	   udelay(DIV_ROUND_UP(2 * 10000000, i2c_dev->busses->bus_clk_rate));
//ZTEBSP DangXiao 20120717,re-init i2c-4 controller when unexpected irq is triggered,end
...
//ZTEBSP DangXiao 20120717,re-init i2c-4 controller when unexpected irq is triggered,start
        if((i2c_dev->msg_add == 0) && (i2c_dev->cont_id = 3) && (i2c_readl(i2c_dev, I2C_INT_MASK) == 0)) {
                if (!i2c_dev->irq_disabled) {
                        disable_irq_nosync(i2c_dev->irq);
                        i2c_dev->irq_disabled = 1;
                }
                tegra_i2c_init(i2c_dev, 1);
				printk("Ninja: i2c-4 unexpect irq,re-init the controller\n");
        }
//ZTEBSP DangXiao 20120717,re-init i2c-4 controller when unexpected irq is triggered,end
	...
	struct i2c_msg *msg, int stop)
{
	...
	if (WARN_ON(ret == 0)) {
		...
		tegra_i2c_init(i2c_dev,0);//ZTEBSP DangXiao 20120718,re-init i2c-4
		return -ETIMEDOUT;
	}
...
static int tegra_i2c_probe(struct platform_device *pdev)
{
	...
	ret = tegra_i2c_init(i2c_dev,0);//ZTEBSP DangXiao 20120718,re-init i2c-4
	if (ret) {

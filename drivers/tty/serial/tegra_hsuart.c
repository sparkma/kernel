...
//zte wuzhengwei 合入NVpatch，解决蓝牙耳机播放视频卡住，NVbug 927947 2012.03.07 begin
static void tegra_start_next_tx(struct tegra_uart_port *t);
static void do_handle_tx_pio(struct tegra_uart_port *t);
//zte wuzhengwei 合入NVpatch，解决蓝牙耳机播放视频卡住，NVbug 927947 2012.03.07 end
...
//#ifndef CONFIG_ARCH_TEGRA_2x_SOC
		...
		if ((lsr & UART_LSR_THRE) != UART_LSR_THRE) {
			dev_vdbg(t->uport.dev, "%s tegra uart tx holding register is full.\n", __func__);
			break;
		}			
#endif
//zte wuzhengwei 合入NVpatch，解决蓝牙耳机播放视频卡住，NVbug 927947 2012.03.07 end
...
//zte wuzhengwei 合入NVpatch，解决蓝牙耳机播放视频卡住，NVbug 927947 2012.03.07 begin
	...
	/* check if tx holding register is empty */
      if ((uart_readb(t, UART_LSR) & UART_LSR_THRE) != UART_LSR_THRE) {
	  	dev_vdbg(t->uport.dev, "%s tegra uart tx holding register is full.\n", __func__);

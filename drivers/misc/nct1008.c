...
struct i2c_client *nct1008_i2c_client = NULL;

signed int nct1008_local_temp(void)
{
	struct i2c_client *client = nct1008_i2c_client;
...
signed int nct1008_ext_temp(void)
{
	struct i2c_client *client = nct1008_i2c_client;
	signed int temp_value = 0;
	s32 data = 0;
...
EXPORT_SYMBOL_GPL(nct1008_local_temp);
EXPORT_SYMBOL_GPL(nct1008_ext_temp);
...
u8 to_be_read_register = 0;

static ssize_t nct1008_register_read(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	u8 data = 0;
	...
	data = i2c_smbus_read_byte_data(client, (u8)to_be_read_register);
	if (data < 0) {
		dev_err(&client->dev, "%s: failed to read register %x"
			, __func__, (u8)to_be_read_register);
		sprintf(buf, "failed to read register %x.\n", to_be_read_register);
		return -EINVAL;
	}
	return sprintf(buf, "the value of register %x is %x.\n", to_be_read_register, data);
}

static ssize_t nct1008_register_write(struct device *dev,
	struct device_attribute *attr, char *buf, size_t count)
{
	...
	if (2 == (strlen(buf)-1)) {
		...
		data = i2c_smbus_read_byte_data(client, (u8)reg);		
	  	if (data < 0) {
	  		dev_err(&client->dev, "%s: failed to read register %x"
	  			, __func__, (u8)reg);
	  		return count;
	...
	err =  i2c_smbus_write_byte_data(client, (u8)reg, (u8)val);
	if (err< 0) {
...
/*[ECID:000000] ZTEBSP wangbing 20120101, make the file attribute be compliant with cts */
static DEVICE_ATTR(register, S_IRUGO|S_IWUSR, nct1008_register_read, nct1008_register_write); 
...
/*[ECID:000000] ZTEBSP wangbing, for thermal sensor shutdown, 20111130*/
#if 0
	value = pdata->ext_range ? EXTENDED_RANGE_MAX : STANDARD_RANGE_MAX;
#else
	value = temperature_to_value(pdata->ext_range, 90);  //70
#endif	

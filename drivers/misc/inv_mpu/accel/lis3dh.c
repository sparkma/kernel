...
//[ECID:000000] ZTEBSP wanghaifei start 20120131, add macro
#define ACCEL_LIS3DH_WHO_AM_I  (0x0F)
#define ACCEL_LIS3DH_ID        (0x33)
//[ECID:000000] ZTEBSP wanghaifei end 20120131, add macro
			   ...
			   const struct i2c_device_id *devid)
{
	...
	int result = 0;
	unsigned char product_id = 0xff; //[ECID:000000] ZTEBSP wanghaifei 20120131, add for id detect
...
//[ECID:000000] ZTEBSP wanghaifei start 20120131, determine whether this driver shold be installed
        result = inv_serial_read(client->adapter, pdata->address,
                                 ACCEL_LIS3DH_WHO_AM_I, sizeof(product_id),
                                 &product_id);
        printk("lis3dh detect 0x%x\n", product_id);
        if (result || (product_id != ACCEL_LIS3DH_ID)) {
                dev_err(&client->adapter->dev, "lis3dh not found\n");
                result = -ENODEV;
                goto out_no_free;
        }   
        printk("lis3dh find\n");
//[ECID:000000] ZTEBSP wanghaifei end 20120131, determine whether this driver shold be installed

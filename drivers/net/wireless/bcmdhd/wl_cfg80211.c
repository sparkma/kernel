...
#define MAX_WAIT_TIME 3000        //wait for long time from 1500 by fanjiankang
...
wl_cfg80211_del_virtual_iface(struct wiphy *wiphy, struct net_device *dev)
{
	...
	printk("Wait for interface delteted completely\n");
	rtnl_unlock();
	msleep(1000);
	rtnl_lock();
	return ret;
	...
	struct key_params *params)
{
    ...
    // zte-modify, yanxiangyang, 2012.4.16, support EAP-SIM + WEP, begin
	if (mac_addr &&
		((params->cipher != WLAN_CIPHER_SUITE_WEP40) && (params->cipher != WLAN_CIPHER_SUITE_WEP104)) ) {
		wl_add_keyext(wiphy, dev, key_idx, mac_addr, params);
		goto exit;
	}
	// zte-modify, yanxiangyang, 2012.4.16, support EAP-SIM + WEP, end
	memset(&key, 0, sizeof(key));
	...
	struct beacon_parameters *info)
{
	...
	if (p2p_is_on(wl) &&
		...
		if ((ssid_ie = bcm_parse_tlvs((u8 *)&info->head[ie_offset],
			info->head_len - ie_offset,
			DOT11_MNG_SSID_ID)) != NULL) {
			// zte-modify, yanxiangyang, 2012.3.6, for wifi hidden ssid, begin
			int hidden = 0;
			if(ssid_ie->len == 0) {
			    printk("yxy ssid_ie->len = 0, need hidden ssid\n");
				hidden = 1;
				// yxy: read again, obtain the real ssid
				ssid_ie = bcm_parse_tlvs((u8 *)ssid_ie->data,
			              info->head_len - ie_offset - 1, 
			              DOT11_MNG_SSID_ID);
				if (ssid_ie != NULL) {
				    printk("yxy got real ssid, ssid_ie->len = %d\n", ssid_ie->len);
				} else {
				    printk("yxy not found real ssid\n");
				}
			}
			// zte-modify, yanxiangyang, 2012.3.6, for wifi hidden ssid, end
			...
			// yxy: set hidden
			if(hidden) {
				printk("yxy try to set closednet = 1\n");
				wldev_iovar_setint(dev, "closednet", 1);
			}
			// zte-modify, yanxiangyang, 2012.3.6, for wifi hidden ssid, end
...
static s32 wl_event_handler(void *data)
{
    ...
    daemonize("wl_event");  // yxy, 2012.5.20, enhance wifi stability 

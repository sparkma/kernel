...
/* Watchdog interval */
uint dhd_watchdog_ms = 1000; // yxy, change Watchdog interval to 1000
...
/* Use polling */
// zte-modify, yanxiangyang, 2012.3.23, enable dhd poll, begin
uint dhd_poll = TRUE;  //FALSE;
// zte-modify, yanxiangyang, 2012.3.23, enable dhd poll, end
...
#endif
                     msleep(300) ;  //added by fanjiankang for wfd error
			netif_stop_queue(ifp->net);
			unregister_netdev(ifp->net);
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	int ret;
      ...
      /* added by fanjiankang for flase ioctl begin */
	if(!dhd->pub.up )
	{
	      DHD_ERROR(("%s: fanjiankang added for false ioctl\n", __FUNCTION__));
	      return BCME_OK;
	}
      /* added by fanjiankang for flase ioctl end */
...
dhd_open(struct net_device *net)
{
	...
	loop:		
			...
			//it's ok if ret = 0, otherwise error happened.
			if(ret) //it is not 0, error happened, retry
			{	
				printk("wl_android_wifi_on fail, retry again...\n");
				loop_num ++;
				if(loop_num < 3)
				{
					wl_android_wifi_off(net);
				...
				printk("wl_android_wifi_on fail, retry failure!!!\n");
			}
		}
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	dhd->op_mode = 0;
#ifdef GET_CUSTOM_MAC_ENABLE
    /* zte-modify, yanxiangyang, 2012.3.12, for wifi mac production, begin */
	ret = dhd_custom_get_mac_address(ea_addr.octet);
    printk("\nyxy dhd_watchdog_ms = %d\n yxy wl_event protect open\n", dhd_watchdog_ms);
	printk("yxy dhd_custom_get_mac_address return : %d\n", ret);
	if (!ret) {
	    printk("\nyxy, %s: dhd_custom_get_mac_address MAC Address 1:  %x:%x:%x:%x:%x:%x \n", __FUNCTION__,ea_addr.octet[0],ea_addr.octet[1],
		ea_addr.octet[2], ea_addr.octet[3],ea_addr.octet[4], ea_addr.octet[5]);
		...
		printk("\nyxy, %s: MAC Address 2:  %x:%x:%x:%x:%x:%x \n", __FUNCTION__,ea_addr.octet[0],ea_addr.octet[1],
		ea_addr.octet[2], ea_addr.octet[3],ea_addr.octet[4], ea_addr.octet[5]);
		/* zte-modify, yanxiangyang, 2012.3.12, for wifi mac production, end */
#ifdef GET_CUSTOM_MAC_ENABLE
	}
#endif /* GET_CUSTOM_MAC_ENABLE */
...
//Roaming SFR
// zte-modify, yanxiangyang, 2012.3.5, for sfr hand over requirement, begin
#ifdef ZTE_FEATURE_WIFI_HAND_OVER
{
	int roam_trigger = -83;
	int roam_delta = 5;
	
	dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER, (char *)&roam_trigger, 4, TRUE, 0);	
	dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_DELTA, (char *)&roam_delta, 4, TRUE, 0);
	printk("RRRR yxy set roam_trigger=%d, roam_delta=%d", roam_trigger, roam_delta);
}
#endif
// zte-modify, yanxiangyang, 2012.3.5, for sfr hand over requirement, end
...
#endif /* WL_CFG80211 */
	setbit(eventmask, WLC_E_ASSOCREQ_IE); //yxy roming feature

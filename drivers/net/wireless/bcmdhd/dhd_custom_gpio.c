...
int dhd_custom_get_mac_address(unsigned char *buf)
{
    ...
    if (IS_ERR(fp))
	{
		printk("yxy %s open file err , code = %d\n", __FUNCTION__, PTR_ERR(fp));
 		return -1;
	}
	...
	filp_close(fp, NULL);	
	macaddr[12] = 0x00;
	printk("yxy read macaddr %s  \n", macaddr);

	for(i = 0; i < 6; i++)
	{
		unsigned int sum = 0;
		memset(tmp, 0, sizeof(tmp));
		memcpy(tmp,macaddr + i*2, 2 );
        if( tmp[0] <='F' && tmp[0] >='A')
        {
            j= tmp[0] -'A'+ 10;
        }
        else if(tmp[0] <='f' && tmp[0] >='a')
        {
            j= tmp[0] - 'a'+ 10;
        }
        else
        ...
        sum = j * 16;
		// printk("yxy tmp[0] = %c j = %02x sum = %02x\n ", c, j, sum );
		if( tmp[1] <='F' && tmp[1] >='A')
        {
            j= tmp[1] -'A'+ 10;
        }
        else if(tmp[1] <='f' && tmp[1] >='a')
        {
            j= tmp[1] - 'a'+ 10;
        }
        else
        {
			...
			sum = sum + j;
		// printk("yxy tmp[1] = %c j = %02x sum = %02x\n ", c, j, sum );

...
static struct device *tps6586x_device = NULL;

int tps6586x_read_register(int reg, unsigned int val)
{
        int ret = 0;
        if(NULL == tps6586x_device)
        {
                return -1;
        }
        ret = tps6586x_read(tps6586x_device, reg, val);
        return ret;
}
...
int tps6586x_write_register(int reg, unsigned int val)
{
        ...
        ret = tps6586x_write(tps6586x_device, reg, val);
        return ret;
}
EXPORT_SYMBOL_GPL(tps6586x_write_register);
...
static int __devinit tps6586x_regulator_probe(struct platform_device *pdev)
{
	...
	//[ECID:0000] ZTEBSP maxiaoping 20120110 disable test print log. --
         //printk("Enter into tps6586x_regulator_probe.\n");
...
//[ECID:000000] ZTEBSP maoke 20111001 start, for skate bring up
	tps6586x_device = pdev->dev.parent;
//[ECID:000000] ZTEBSP maoke 20111001 end, for skate bring up
...
//[ECID:0000] ZTEBSP maxiaoping 20111011 for LDOs enable/disable
int pm_tps6586x_LDO7_switch_ctl(bool onoff)
{
	int ret = 0;
         int val = 0;
         int addr1,addr2;
	if(NULL == tps6586x_device)
	{
		printk(KERN_NOTICE "PM_DEBUG_MXP: tps6586x_enable_LDO7 NULL == tps6586x_device.\r\n");
		return 0;
	}
	addr1 = 0x12;
	addr2 = 0x13;
	if(true == onoff)
	{	
      		/*As the LDO enable just need one of the reg 0x12 or 0x13 bit is set,so we set any of it here */
      		ret = tps6586x_read(tps6586x_device, addr1, &val);//0x12
       		val |= 0x20;//LDO7_ONC
    		ret = tps6586x_write(tps6586x_device, addr1, val);
	}
	else
	{
		/*As the LDO disable  need all two regs 0x12 and 0x13 bits is cleared,so we clear all of them here */
      		ret = tps6586x_read(tps6586x_device, addr1, &val);//0x12
       		val &= (~(0x20));//LDO7_ONC
    		ret = tps6586x_write(tps6586x_device, addr1, val);
			
     		ret = tps6586x_read(tps6586x_device, addr2, &val);//0x13
		val &= (~(0x20));//LDO7_OND
  		ret = tps6586x_write(tps6586x_device, addr2, val);    
	}
	return ret;
}

int pm_tps6586x_LDO0_switch_ctl(bool onoff)
{
	int ret = 0;
         int val = 0;
         int addr1,addr2;
	if(NULL == tps6586x_device)
	{
		printk(KERN_NOTICE "PM_DEBUG_MXP: tps6586x_enable_LDO0 NULL == tps6586x_device.\r\n");
		return 0;
	}
	addr1 = 0x12;
	addr2 = 0x13;
...
//[ECID:0000] ZTEBSP maxiaoping 20111011 for LDOs enable/disable end.

//[ECID:0000] ZTEBSP maxiaoping 20120130 for vibrator driver,start
//[ECID:0000] ZTEBSP maxiaoping 20111019 for vibrator driver,start
int tps6586x_vibrator_on(void)
{
    int ret = 0;
    uint8_t val = 0;
    uint8_t val2 = 0;
    ...
    ret = tps6586x_read(tps6586x_device, 0x5b, &val);
    if(ret)
    {
        printk("failed to read pmu reg:0x5b\n");
        return ret;
    } 
    //val |= 0xff;  //freq=0.75K,duty cycle=100%
    val |= 0xFE;      /*freq=0.75K,   duty cycle=93.75%*/
    ret = tps6586x_write(tps6586x_device, 0x5b, val);
    if(ret)
    {
        printk("failed to write pmu reg:0x5b\n");
        return ret;
    }
    return ret;
}
//[ECID:0000] ZTEBSP maxiaoping 20111019 for vibrator driver,end
//[ECID:0000] ZTEBSP maxiaoping 20120130 for vibrator driver,end

EXPORT_SYMBOL(tps6586x_vibrator_on);
int tps6586x_vibrator_off(void)
{
    int ret = 0;
    uint8_t val = 0;
    if(NULL == tps6586x_device)
...
EXPORT_SYMBOL(tps6586x_vibrator_off);

//[ECID:0000] ZTEBSP maxiaoping 20111019 vibrator driver end.

				...
				struct usb_device *udev, unsigned int delay)
{
	...
	for (delay_time = 0;
		...
		dev_dbg(hub->intfdev, "%s ret = %d, portstatus = 0x%X, portchange = 0x%X\n", __func__, ret, portstatus, portchange);
		if (ret < 0)
			return ret;
...
retry_port_reset:  // add nv patch
	...
	if (oldspeed != USB_SPEED_UNKNOWN && oldspeed != udev->speed) {
		// add nv patch begin
		/* Tegra WAR: If speed changed from HS to FS during modem usb mac hw reset, retry with port reset again. */
		if (retry++ < 4) {
			dev_dbg(&udev->dev, "device reset changed speed, retry (%d).\n", retry);
			goto retry_port_reset;
		} else {

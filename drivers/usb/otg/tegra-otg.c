...
#include <linux/err.h>
//ZTEBSP DangXiao 20120322,USB 0.5sec-wakelock,start 
#include <linux/wakelock.h>
//ZTEBSP DangXiao 20120322,USB 0.5sec-wakelock,end
...
//[ECID:000000] ZTEBSP zhangjing 20120330 start, according to nvidia's martin for avoid otg.		
		//	to = OTG_STATE_A_HOST;
			to = OTG_STATE_B_PERIPHERAL;
//[ECID:000000] ZTEBSP zhangjing 20120330 end, according to nvidia's martin for avoid otg.	
	}
	...
	if (to != OTG_STATE_UNDEFINED) {
		...
		if (to == OTG_STATE_A_SUSPEND) {
			//ZTEBSP DangXiao 20120517,USB 0.5sec-wakelock,start 
			if(from != to)
				wake_lock_timeout(&tegra->usb_disconnect_wake_lock,HZ/2);
			//ZTEBSP DangXiao 20120517,USB 0.5sec-wakelock,end
			if (from == OTG_STATE_A_HOST)
...
static int tegra_otg_probe(struct platform_device *pdev)
{
	...
	if (!ehci_pdata->default_enable)
		clk_disable(tegra->clk);
	//ZTEBSP DangXiao 20120322,USB 0.5sec-wakelock,start 
	wake_lock_init(&tegra->usb_disconnect_wake_lock, WAKE_LOCK_SUSPEND, "usb_disconnection");
	//ZTEBSP DangXiao 20120322,USB 0.5sec-wakelock,end
...
err_clk:
	//ZTEBSP DangXiao 20120322,USB 0.5sec-wakelock,start 
	wake_lock_destroy(&tegra->usb_disconnect_wake_lock);
	//ZTEBSP DangXiao 20120322,USB 0.5sec-wakelock,end

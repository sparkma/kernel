...
static int com_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (com->port.in->driver_data) {
		...
		com->port.in_desc = ep_choose(cdev->gadget,
				com->hs.in, com->fs.in);
		com->port.out_desc = ep_choose(cdev->gadget,
				com->hs.out, com->fs.out);
	}
...
com_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	com->fs.in = usb_find_endpoint(com_fs_function,
			f->descriptors, &com_fs_in_desc);
	com->fs.out = usb_find_endpoint(com_fs_function,
			f->descriptors, &com_fs_out_desc);
	...
	if (gadget_is_dualspeed(c->cdev->gadget)) {
		...
		com->hs.in = usb_find_endpoint(com_hs_function,
				f->hs_descriptors, &com_hs_in_desc);
		com->hs.out = usb_find_endpoint(com_hs_function,
				f->hs_descriptors, &com_hs_out_desc);
	}

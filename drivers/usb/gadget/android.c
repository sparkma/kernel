...
#include "u_serial.c"
#include "f_acm.c"
//[ECID:000000] ZTEBSP zhangjing 20120218 start, for adding com port
#include "f_com.c"
//[ECID:000000] ZTEBSP zhangjing 20120218 end, for adding com port
#include "f_adb.c"
#include "f_mtp.c"
...
//[ECID:000000] ZTEBSP zhangjing 20120218 start, for adding com port
#define COM_INSTANCES_NUM	2

static int com_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	/* nothing to do - initialization is handled by acm_function_init */
	/* return gserial_setup(cdev->gadget, MAX_ACM_INSTANCES); */
	return 0;
}

static void com_function_cleanup(struct android_usb_function *f)
{
	/* nothing to do - cleanup is handled by acm_function_cleanup */
	/* gserial_cleanup(); */
}

static int com_function_bind_config(struct android_usb_function *f, struct usb_configuration *c)
{
	int i;
	int ret = 0;

	for (i = 0; i < COM_INSTANCES_NUM; i++) {
		ret = com_bind_config(c, i);
		if (ret) {
			pr_err("Could not bind com%u config\n", i);
			break;
		}
	}
...
static struct android_usb_function com_function = {
	.name		= "com",
	.init		= com_function_init,
	.cleanup	= com_function_cleanup,
	.bind_config	= com_function_bind_config,
};
//[ECID:000000] ZTEBSP zhangjing 20120218 end, for adding com port
...
//[ECID:000000] ZTEBSP zhangjing 20120218 start, for adding usb
static struct mass_storage_function_config *mass_storage_config;
//[ECID:000000] ZTEBSP zhangjing 20120218 end, for adding usb

...
//zte_modify nv patch begin
		...
		if (tegra->port_resuming) {
			...
			if (handshake(ehci, status_reg, (PORT_SUSPEND | PORT_RESUME), 0, 25000)) {
				ehci_err(ehci, "timeout waiting for SUSPEND to clear\n");
			}
			tegra->port_resuming = 0;
			tegra_usb_phy_postresume(tegra->phy, false);
			if (tegra->phy->usb_phy_type == TEGRA_USB_PHY_TYPE_UTMIP ||
			    tegra->phy->usb_phy_type == TEGRA_USB_PHY_TYPE_NULL_ULPI) {
				ehci_dbg(ehci, "GetPortStatus, run host & enable interrupt\n");
...
//zte_modify nv patch end
	 ...
	 */
		...
		if (tegra->phy->usb_phy_type == TEGRA_USB_PHY_TYPE_UTMIP ||
		    tegra->phy->usb_phy_type == TEGRA_USB_PHY_TYPE_NULL_ULPI) {
			ehci_dbg(ehci, "ClearPortFeature USB_PORT_FEAT_SUSPEND, disable interrupt & stop host\n");
			/* disable interrupts */
			ehci_writel(ehci, 0, &ehci->regs->intr_enable);
			/* Disable RUN bit. */
			ehci->command &= ~CMD_RUN;
...
//zte_modify nv patch begin
	if ((tegra->port_speed > TEGRA_USB_PHY_PORT_SPEED_HIGH) || (hsic) ||
	    (null_ulpi)) {
		/* waiting for MAC reset complete at modem side, bind with modem FW 1.5.9+ */
		msleep(20);
		goto restart;
	}
//zte_modify nv patch end
...
//[ECID:000000]ZTEBSP wangxiaomei  20120323 start, for nv bug 952153
	mutex_lock(&tegra->tegra_ehci_hcd_mutex);
...
//zte_modify nv patch end
	...
	if (!tegra->power_down_on_bus_suspend && !tegra->host_resumed) {
		pr_err("%s host never resumed!\n", __func__);
		mutex_unlock(&tegra->tegra_ehci_hcd_mutex);
		return 0;
	}
	//mutex_lock(&tegra->tegra_ehci_hcd_mutex);
//[ECID:000000]ZTEBSP wangxiaomei  20120323 end, for nv bug 952153
...
static int tegra_ehci_probe(struct platform_device *pdev)
{
	...
	if (pdata->phy_type == TEGRA_USB_PHY_TYPE_NULL_ULPI) {
		/* reset the controller to start fresh on every register */

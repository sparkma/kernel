...
#include "dsi_regs.h"
#include "dsi.h"
//[ECID:000000] ZTEBSP wangminrong start 20120316 add lcd reset
#define TEGRA_GPIO_PV7		175
static unsigned char lcd_data = 0;
//[ECID:000000] ZTEBSP wangminrong end 20120316 add lcd reset
					...
					u32 n_cmd)
{
	u32 i;
	int err;
//ZTEBSP DangXiao 20120228, DSI panel add reset each time,start
        u8 buffer[10];
{
	static int InitFlag = 0;
	if(cmd->sp_len_dly.sp.data0!=0x28 && InitFlag == 1)
	{
		...
		gpio_set_value(175,1);
		mdelay(30);
		printk("=====  Ninja: reset panel ========\n");
	}
	if(InitFlag == 0)
	{
		InitFlag = 1;
		printk("=====  Ninja: First boot time, we do not reset and init panel ========\n");
		return 0;
	}
}	
//ZTEBSP DangXiao 20120228, DSI panel add reset each time,end

    //tegra_dsi_read_data(dc, dsi, 10, 0xDA, buffer);
    //printk("[tegra_dsi_send_panel_cmd ]tegra_dsi_read_data Reg:0xDA --> %x ,%x, %x   -------\n", buffer[0], buffer[1], buffer[2]);
	
	//tegra_dsi_read_data(dc, dsi, 10, 0x0A, buffer);
	//printk("[tegra_dsi_send_panel_cmd ]tegra_dsi_read_data Reg:0x0A --> %x ,%x, %x	 -------\n", buffer[0], buffer[1], buffer[2]);
...
static void tegra_dc_dsi_disable(struct tegra_dc *dc)
{
	...
	if (dsi->status.dc_stream == DSI_DC_STREAM_ENABLE)
		tegra_dsi_stop_dc_stream_at_frame_end(dc, dsi);
        mdelay(50); //ZTEBSP DangXiao 20120229, nvidia martin add at 2011.12.17

	if (dsi->info.power_saving_suspend) {
		printk("Ninja: panel enter deep sleep! \n");

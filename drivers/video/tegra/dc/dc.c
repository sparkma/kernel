...
extern struct tegra_pingroup_config lcd_lm1_pinmux;
extern void tegra_pinmux_config_pingroup(const struct tegra_pingroup_config *config);
extern void tegra_gpio_disable(int gpio);
#define bl_pwm_ctl TEGRA_GPIO_PW1
//[ECID:0000] ZTEBSP maxiaoping 20120105 --
...
//[ECID:000000]ZTE BSP khl modify for clk_tree shutup start 2012.04.19
...
static void tegra_dc_program_bandwidth(struct tegra_dc *dc)
{
	...
	if (dc->emc_clk_rate != dc->new_emc_clk_rate) {
		...
		if (!dc->emc_clk_rate && !tegra_is_clk_enabled(dc->emc_clk)) /* going from 0 to non-zero */
...
//[ECID:000000]ZTE BSP khl modify for clk_tree shutup end 2012.04.19
static int tegra_dc_set_dynamic_emc(struct tegra_dc_win *windows[], int n)
{
	...
	if (tegra_dc_has_multiple_dc())
		new_rate = ULONG_MAX;
	//ZTEBSP DangXiao 20120528,EMC rate set to 150MHz, start
	dc->new_emc_clk_rate = tegra_dc_get_default_emc_clk_rate(dc);
	if(new_rate > dc->new_emc_clk_rate)	//Only allow 150MHz or more
		dc->new_emc_clk_rate = new_rate;
	//ZTEBSP DangXiao 20120528,EMC rate set to 150MHz, end
	return 0;
}
...
int tegra_dc_update_windows(struct tegra_dc_win *windows[], int n)
{
	...
	//ZTEBSP DangXiao 20120629 NVIDIA Patch on Google Voice Search Display Blink,start
	val = tegra_dc_readl(dc, DC_CMD_INT_MASK);
	val &= ~(FRAME_END_INT | V_BLANK_INT | ALL_UF_INT);
	tegra_dc_writel(dc, val, DC_CMD_INT_MASK);
	//ZTEBSP DangXiao 20120629 NVIDIA Patch on Google Voice Search Display Blink,end
...
void tegra_dc_setup_clk(struct tegra_dc *dc, struct clk *clk)
{
	...
	if (dc->out->type == TEGRA_DC_OUT_HDMI) {
		...
		//ZTEBSP DangXiao 20120416, for HDMI & MHL clock patch,start
		if (dc->mode.pclk > 70000000)
			rate = 445500000;  //rate = 594000000;
		...
		//ZTEBSP DangXiao 20120416, for HDMI & MHL clock patch,end
		else
			rate = 504000000;
...
tegra_dc_config_pwm(struct tegra_dc *dc, struct tegra_dc_pwm_params *cfg)
{
	...
	//printk(KERN_NOTICE "PM_DEBUG_MXP: Enter tegra_dc_config_pwm\n");
	
	if(run_once == 1)
	{
		//gpio_set_value(ventana_bl_enb, 0);
		//gpio_free(ventana_bl_enb);
		//here we need to clear the gpio function first,config it to spio. 
		tegra_gpio_disable(bl_pwm_ctl);
		tegra_pinmux_config_pingroup(&lcd_lm1_pinmux);
		run_once =0;
	}
	//if(run_once == 1)
	{	
		//printk(KERN_NOTICE "PM_DEBUG_MXP: Enter run_once\n");
		/* 1. Set LCD_m1 as LCD1 - Done in pinmux configuration */
        		/* 2. Set DC_COM_PIN_OUTPUT_SELECT5_0 bit[6:4] as PM1 function */
		reg = tegra_dc_readl(dc, DC_COM_PIN_OUTPUT_SELECT5);
		//printk(KERN_NOTICE "PM_DEBUG_MXP: DC_COM_PIN_OUTPUT_SELECT5=0x%x\n",reg);
		reg &= (~0x70); 
		reg |= (0x3 << 4);
		tegra_dc_writel(dc, reg, DC_COM_PIN_OUTPUT_SELECT5);
		//printk(KERN_NOTICE "PM_DEBUG_MXP: DC_COM_PIN_OUTPUT_SELECT5=0x%x\n",reg);

		/* 3. Set DC_CMD_DISPLAY_POWER_CONTROL_0 bit[18] to enable */
		reg = tegra_dc_readl(dc, DC_CMD_DISPLAY_POWER_CONTROL);
		//printk(KERN_NOTICE "PM_DEBUG_MXP: DC_CMD_DISPLAY_POWER_CONTROL=0x%x\n",reg);
		reg |= (1<<18);
		tegra_dc_writel(dc, reg, DC_CMD_DISPLAY_POWER_CONTROL);
		//printk(KERN_NOTICE "PM_DEBUG_MXP: DC_CMD_DISPLAY_POWER_CONTROL=0x%x\n",reg);
		//run_once =0;
	}
	...
	/*[ECID:000000] ZTEBSP zhangbo comment it, start*/
	...
	/*[ECID:000000] ZTEBSP zhangbo comment it, end*/

	//printk(KERN_NOTICE "PM_DEBUG_MXP: ctrl=0x%x\n",ctrl);
	//printk(KERN_NOTICE "PM_DEBUG_MXP: cfg->duty_cycle=0x%x\n",cfg->duty_cycle);
	...
	case TEGRA_PWM_PM1:
		...
		//tegra_dc_writel(dc, out_sel, DC_COM_PIN_OUTPUT_SELECT5);
		/*[ECID:000000] ZTEBSP zhangbo comment it, end*/
		//reg = tegra_dc_readl(dc, DC_COM_PM1_CONTROL);
		//printk(KERN_NOTICE "PM_DEBUG_MXP: DC_COM_PM1_CONTROL=0x%x\n",reg);
		tegra_dc_writel(dc, ctrl, DC_COM_PM1_CONTROL);
		//reg = tegra_dc_readl(dc, DC_COM_PM1_CONTROL);
		//printk(KERN_NOTICE "PM_DEBUG_MXP: DC_COM_PM1_CONTROL=0x%x\n",reg);
		
		//reg = tegra_dc_readl(dc, DC_COM_PM1_DUTY_CYCLE);
		//printk(KERN_NOTICE "PM_DEBUG_MXP: DC_COM_PM1_DUTY_CYCLE=0x%x\n",reg);
...
static bool _tegra_dc_controller_enable(struct tegra_dc *dc)
{
	...
	//ZTEBSP DangXiao 20120525,NV patch,for the sake of safty,start
	tegra_dc_init(dc);
	
	enable_dc_irq(dc->irq);
	//ZTEBSP DangXiao 20120525,NV patch,for the sake of safty,end
...
//ZTEBSP DangXiao 20120616,start
extern unsigned int panel_early_suspend_flag;
static void tegra_dc_shutdown(struct nvhost_device *ndev)
{
	unsigned i;
	struct tegra_dc *dc =nvhost_get_drvdata(ndev);
	struct tegra_dc_pwm_params backlight_cfg;

	printk("Ninja: enter %d tegra_dc_shutdown!\n", ndev->id);

	backlight_cfg.which_pwm = TEGRA_PWM_PM1;
	backlight_cfg.duty_cycle = 0;
	if (dc)
	{
		printk("Ninja: turn off Backlights!\n");
		tegra_dc_config_pwm(dc, &backlight_cfg);
	}
	
	if(panel_early_suspend_flag==1)
	{
		printk("Ninja: panel already suspended!\n");
		return 0;
	}
	if(dc->out_ops==&tegra_dc_dsi_ops)
	{	
		tegra_dc_writel(dc, 0, DC_CMD_INT_MASK);
		tegra_dc_writel(dc, 0, DC_CMD_INT_ENABLE);
		disable_irq(dc->irq);
		printk("Ninja: disable_irq! \n");
		if (dc->out_ops && dc->out_ops->disable)
			dc->out_ops->disable(dc);
	}
	printk("Ninja: dc shutdown end!\n");
	return 0;
}
//ZTEBSP DangXiao 20120616,end
...
//ZTEBSP DangXiao 20120615,start
	.shutdown = tegra_dc_shutdown,
//ZTEBSP DangXiao 20120615,end

...
//[ECID:000000]ZTEBSP DangXiao 20120129 start,add patch for LCD blink when wake up by pwrkey,update
unsigned int fb_blank_flag = 1;
...
static int tegra_fb_blank(int blank, struct fb_info *info)
{
	...
	case FB_BLANK_UNBLANK:
		...
		if (tegra_fb->win->dc->ndev->id == 0)  //Ninja: only unblank fb0(main display)
		{
			fb_blank_flag = 1;
		}
		return 0;
	...
	case FB_BLANK_POWERDOWN:
		...
		tegra_dc_disable(tegra_fb->win->dc);
			if (tegra_fb->win->dc->ndev->id == 0) //Ninja: only flush fb0(main display)
				fb_blank_flag = 0;
		return 0;

...
static void aic3262_dump_page(struct snd_soc_codec *codec, u8 page);
#endif
...
static void aic3262_dump_page(struct snd_soc_codec *codec, u8 page)
{
	...
	if(aic3262->bus_type == SND_SOC_SPI) {
		data = 0x01;
		spi_write_then_read(codec->control_data,&data,1,test_page_array,127);
	} else {
		data = 0x0;
		i2c_master_send(codec->control_data, data, 1);
		i2c_master_recv(codec->control_data, test_page_array, 8);
	}
			   ...
			   size_t count, loff_t * offset)
{
	...
	if(aic3262->bus_type == SND_SOC_SPI) {
		for(i=0;i<count;i++) {
			rd_data[i] = codec->hw_read(codec, (reg_addr + j));
			j++;
		}
		size = count;
	} else {
		/* Send the address to device thats is to be read */
		if (i2c_master_send(codec->control_data, &reg_addr, 1) != 1) {
			dprintk("Can not write register address\n");
			return -1;
		}
		/* read the codec device registers */
		size = i2c_master_recv(codec->control_data, rd_data, count);
	}
#ifdef DEBUG

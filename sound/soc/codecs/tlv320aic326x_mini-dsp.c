...
int dsp_reg_write (struct snd_soc_codec *codec , u8 book, unsigned int reg, unsigned int val)
{
	...
	if(aic3262->bus_type == SND_SOC_SPI) {
		if (codec->hw_write(codec->control_data, buf,2) < 0) {
	...
	do {
		do {
			...
			if(aic3262->bus_type == SND_SOC_SPI) {
				...
				spi_cs_en(0);
				//codec->hw_write(codec->control_data, spi_data[count].buf,spi_data[count].len);
...
struct process_flow{
	...
	
	{
			...
			{ 0, 0, 0, 0},
			{ 0, 0, 0, 0},
  			{ ARRAY_SIZE(filter_off_miniDSP_A_reg_values), filter_off_miniDSP_A_reg_values,
				ARRAY_SIZE(filter_off_miniDSP_D_reg_values),filter_off_miniDSP_D_reg_values },
			{ ARRAY_SIZE(filter_on_miniDSP_A_reg_values),filter_on_miniDSP_A_reg_values , 
				ARRAY_SIZE(filter_on_miniDSP_D_reg_values),filter_on_miniDSP_D_reg_values },
...
/*
* change_codec_power_status
* Function to take the Codec to a known state while switching the
* processflow. This function powers off the DAC/ADC based on the power_mask
* parameter and then turns them ON after the process-flow switching is completed.
*/
...
//[ECID:000000] ZTEBSP jiaobaocun 201200312 start, for codec
	if(new_mode == 1) {

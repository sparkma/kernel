...
#include "../../../../arch/arm/mach-tegra/board-whistler.h"
 ...
 *****************************************************************************
 */
//[ECID:000000] ZTEBSP jiaobaocun 201200305 start, for headset  
DEFINE_MUTEX(codec_io_mutex);
//[ECID:000000] ZTEBSP jiaobaocun 201200305 end, for headset  
static u8 aic3262_reg_ctl;
bool be_headset_state = false;
#ifdef AIC3262_TiLoad
	extern int aic3262_driver_init(struct snd_soc_codec *codec);
#endif
/*In support of custom DAPM TLV implementation*/
extern int update_power_tlv(struct snd_soc_dapm_widget *widget,
...
/*In support of custom DAPM TLV implementation*/
#define SOC_DAPM_SINGLE_TLV_M(xname, reg, shift, max, invert, tlv_array) \
{	.iface = SNDRV_CTL_ELEM_IFACE_MIXER, .name = xname, \
	.info = snd_soc_info_volsw, \
	.access = SNDRV_CTL_ELEM_ACCESS_TLV_READ | SNDRV_CTL_ELEM_ACCESS_READWRITE,\
	.tlv.p = (tlv_array), \
	.get = snd_soc_dapm_get_volsw, .put = snd_soc_dapm_put_volsw_m, \
...
#endif
/**
 * snd_soc_info_volsw_m - single mixer info callback
 ...
 */
int snd_soc_info_volsw_m(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
...
EXPORT_SYMBOL_GPL(snd_soc_info_volsw_m);

/**
 * snd_soc_dapm_get_volsw_m - dapm mixer get callback
 * @kcontrol: mixer control
 ...
 * Returns 0 for success.
 */
int snd_soc_dapm_get_volsw_m(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
...
EXPORT_SYMBOL_GPL(snd_soc_dapm_get_volsw_m);
/**
 * snd_soc_dapm_put_volsw_m - dapm mixer custom set callback
 * @kcontrol: mixer control
 * @ucontrol: control element information
 *
 ...
 * Returns 0 for success.
 */
int snd_soc_dapm_put_volsw_m(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_dapm_widget *widget = snd_kcontrol_chip(kcontrol);
	struct soc_mixer_control *mc =
	...
	//printk(" %s: start of custom TLV function\n",__func__);
	...
	if (change) {
		...
		if (val){
			/* new connection */
			connect = invert ? 0:1;
			printk("%s: %s connected \n", __func__, widget->name);
			}
		else{
			/* old connection must be powered down */
			connect = invert ? 1:0;
			printk("%s: %s not connected \n", __func__, widget->name);
			}
		#else
			#if 1
			if(invert){
...
//		dapm_mixer_update_power(widget, kcontrol, connect);
		update_power_tlv(widget, kcontrol, connect);
		widget->dapm->update = NULL;
		//printk(" %s: END of custom TLV function\n",__func__);
	}

	mutex_unlock(&widget->codec->mutex);
	return 0;
}
EXPORT_SYMBOL_GPL(snd_soc_dapm_put_volsw_m);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!strcmp(kcontrol->id.name, "PCM Playback Volume")) {
		...
		mask = 0x7F;
		shift = 0;
	} else if (!strcmp(kcontrol->id.name, "Receiver Amplifier Volume")) {
		mask = 0x3F;
		shift = 0;
...
static const struct snd_kcontrol_new aic3262_snd_controls[] = {
	...
	SOC_DOUBLE_R_N("Receiver Amplifier Volume", REC_AMP_CNTL_R5, RAMPR_VOL, 0, 36, 0), 
...
//[ECID:000000] ZTEBSP jiaobaocun 20120301 start, for Codec
...
//[ECID:000000] ZTEBSP jiaobaocun 20120301 end, for Codec
	SOC_SINGLE("HP_DEPOP", HP_DEPOP, 2, 16, 0),
...
//just different because  N890 and 943T use different sample rate  in modem.
...
//[ECID:000000] ZTEBSP jiaobaocun 201200305 end, for headset  
	...
	{12288000, 48000, 1, 8, 52, 128, 2, 8, 128, 2, 8, 4,
		{{0, 60, 0}, {0, 61, 0} } },
//[ECID:000000] ZTEBSP jiaobaocun 20120227 start, for Codec
	{12000000, 48000, 1, 8, 1920, 128, 2, 8, 128, 2, 8, 4,
         {{0, 60, 0}, {0, 61, 0}}},
//[ECID:000000] ZTEBSP jiaobaocun 20120227 end, for Codec
#else
	/* 48k rate */
...
//[ECID:000000] ZTEBSP jiaobaocun 201200305 start, for headset  
	...
	*/
	...
	/* Read the corresponding ASI DAI Interface Register */
	...
	*/
	...
	DBG(" +++++Count of ASI%d active is %d +++++\n",dai->id, aic3262->asiCtxt[dai->id - 1].asi_active);
	return 0;
}
...
static int aic3262_multi_i2s_set_clkdiv(struct snd_soc_dai *codec_dai, int div_id, int div)
{
	...
	DBG(KERN_INFO "#%s: DAI ID %d Page %d Register %d Divider_Val %d Final_Value 0x%x\n",
...
static const struct aic3262_configs aic3262_reg_init[] = {
	...
	//[ECID:000000] ZTEBSP jiaobaocun 20120323 start, for codec  
	{0, PASI_DAC_DP_SETUP,  0x02},	/*DAC */
	{0, DAC_MVOL_CONF,  0x0C},	/*DAC should be muted during initialization */
	/* set default volumes */
	{0, DAC_LVOL, 0x00},
	{0, DAC_RVOL, 0x00},
	{0, HPL_VOL,  0x80},
	...
	{0, SPK_AMP_CNTL_R4, 0x00},
	{0, REC_AMP_CNTL_R5, 0x39},
	//[ECID:000000] ZTEBSP jiaobaocun 20120323 end, for codec  
	...
	{0, REF_PWR_DLY, 0x01},
	//{0, CM_REG, 0x00},	/*CM - default{0, CM_REG, 0xc0},*/
	...
	{0, LMIC_PGA_MIN, 0x40},	/* No Internal CM Connection by default */
	{0, RMIC_PGA_PIN, 0x0},	/*IN1_R select - - 10k -RMIC_PGA_P*/
	{0, RMIC_PGA_MIN, 0x0},	/*CM to RMICPGA_M*/
	{0, MIC_PWR_DLY , 0x32},	/*LMIC-PGA-POWERUP-DELAY - default*/
	...
	/* Charge-pump Registers Configuration */
	{0, CHRG_PUMP_CNTL_1, 0x28},
	{0, CHRG_PUMP_CNTL_2, 0x3E},
	{0, CHRG_PUMP_CNTL_3, 0x30},
...
//[ECID:000000] ZTEBSP jiaobaocun 20120310 start, for codec
/* Left HPL Mixer */
static const struct snd_kcontrol_new hpl_output_mixer_controls[] = {
	SOC_DAPM_SINGLE("MAL Switch", HP_AMP_CNTL_R1, 7, 1, 0),
	SOC_DAPM_SINGLE("LDAC Switch", HP_AMP_CNTL_R1, 5, 1, 0),
	SOC_DAPM_SINGLE_TLV_M("LOL-B1 Volume", HP_AMP_CNTL_R2, 0,
							0x7f, 0, lo_hp_tlv),
};

/* Right HPR Mixer */
static const struct snd_kcontrol_new hpr_output_mixer_controls[] = {
	SOC_DAPM_SINGLE_TLV_M("LOR-B1 Volume", HP_AMP_CNTL_R3, 0,
							0x7f, 0, lo_hp_tlv),
...
//[ECID:000000] ZTEBSP jiaobaocun 20120310 end, for codec
...
static const struct snd_kcontrol_new spkl_output_mixer_controls[] = {
	...
	SOC_DAPM_SINGLE_TLV_M("LOL Volume", SPK_AMP_CNTL_R2, 0, 0x7f,0,
								lo_hp_tlv),
...
static const struct snd_kcontrol_new spkr_output_mixer_controls[] = {
	SOC_DAPM_SINGLE_TLV_M("LOR Volume", SPK_AMP_CNTL_R3, 0, 0x7f, 0,
								lo_hp_tlv),
...
static const struct snd_kcontrol_new rec_output_mixer_controls[] = {
	SOC_DAPM_SINGLE_TLV_M("LOL-B2 Volume", RAMP_CNTL_R1, 0, 0x7f,0,
							lo_hp_tlv),
	SOC_DAPM_SINGLE_TLV_M("IN1L Volume", IN1L_SEL_RM, 0, 0x7f, 1, lo_hp_tlv),
	SOC_DAPM_SINGLE_TLV_M("IN1R Volume", IN1R_SEL_RM, 0, 0x7f, 1, lo_hp_tlv),
	SOC_DAPM_SINGLE_TLV_M("LOR-B2 Volume", RAMP_CNTL_R2, 0,0x7f, 0,lo_hp_tlv),
};

/* Left Input Mixer */
static const struct snd_kcontrol_new left_input_mixer_controls[] = {
	SOC_DAPM_SINGLE("IN1L Switch", LMIC_PGA_PIN, 6, 0x03, 0),
	SOC_DAPM_SINGLE("IN2L Switch", LMIC_PGA_PIN, 4, 0x03, 0),
	SOC_DAPM_SINGLE("IN3L Switch", LMIC_PGA_PIN, 2, 0x03, 0),
	SOC_DAPM_SINGLE("IN4L Switch", LMIC_PGA_PM_IN4, 5, 1, 0),
	SOC_DAPM_SINGLE("IN1R Switch", LMIC_PGA_PIN, 0, 0x03, 0),
	SOC_DAPM_SINGLE("IN2R Switch", LMIC_PGA_MIN, 4, 0x03, 0),
	SOC_DAPM_SINGLE("IN3R Switch", LMIC_PGA_MIN, 2, 0x03, 0),
	SOC_DAPM_SINGLE("IN4R Switch", LMIC_PGA_PM_IN4, 4, 1, 0),
	SOC_DAPM_SINGLE("CM2L Switch", LMIC_PGA_MIN, 0, 0x03, 0),
	SOC_DAPM_SINGLE("CM1L Switch", LMIC_PGA_MIN, 6, 0x03, 0),
};

/* Right Input Mixer */
static const struct snd_kcontrol_new right_input_mixer_controls[] = {
	SOC_DAPM_SINGLE("IN1R Switch", RMIC_PGA_PIN, 6, 0x03, 0),
	SOC_DAPM_SINGLE("IN2R Switch", RMIC_PGA_PIN, 4, 0x03, 0),
	SOC_DAPM_SINGLE("IN3R Switch", RMIC_PGA_PIN, 2, 0x03, 0),
	SOC_DAPM_SINGLE("IN4R Switch", RMIC_PGA_PM_IN4, 5, 1, 0),
	SOC_DAPM_SINGLE("IN2L Switch", RMIC_PGA_PIN, 0, 0x03, 0),
	SOC_DAPM_SINGLE("IN1L Switch", RMIC_PGA_MIN, 4, 0x03, 0),
	SOC_DAPM_SINGLE("IN3L Switch", RMIC_PGA_MIN, 2, 0x03, 0),
	SOC_DAPM_SINGLE("IN4L Switch", RMIC_PGA_PM_IN4, 4, 1, 0),
	SOC_DAPM_SINGLE("CM1R Switch", RMIC_PGA_MIN, 6, 0x03, 0),
	SOC_DAPM_SINGLE("CM2R Switch", RMIC_PGA_MIN, 0, 0x03, 0),
};
...
static int spk_driver_event(struct snd_soc_dapm_widget *w,
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	if(event & SND_SOC_DAPM_PRE_PMU) {
		if(aic3262->fm_mode_state == true) {
			if(!strcmp(w->name, "HPL Driver")) {
				aic3262->fm_HPL_gain = snd_soc_read(codec, HPL_VOL);
				snd_soc_update_bits(codec, HPL_VOL, 0x3f, 0x39);
				mdelay(1);
				snd_soc_update_bits(codec, HP_AMP_CNTL_R1, 0x80, 0x00);
				mdelay(1);
			}
			if(!strcmp(w->name, "HPR Driver")) {
				aic3262->fm_HPR_gain = snd_soc_read(codec, HPR_VOL);
				snd_soc_update_bits(codec, HPR_VOL, 0x3f, 0x39);
				mdelay(1);
				snd_soc_update_bits(codec, HP_AMP_CNTL_R1, 0x40, 0x00);
				mdelay(1);
			}
		}
	}
	...
	if(event & SND_SOC_DAPM_POST_PMU) {
		if(aic3262->fm_mode_state == true) {
			if(!strcmp(w->name, "HPL Driver")) {
				snd_soc_update_bits(codec, HP_AMP_CNTL_R1, 0x80, 0x80);
				mdelay(1);
				snd_soc_update_bits(codec, HPL_VOL, 0x3f, aic3262->fm_HPL_gain);
				mdelay(1);
				}
			if(!strcmp(w->name, "HPR Driver")) {	
				snd_soc_update_bits(codec, HP_AMP_CNTL_R1, 0x40, 0x40);
				mdelay(1);
				snd_soc_update_bits(codec, HPR_VOL, 0x3f, aic3262->fm_HPR_gain);
...
static int rec_power_event(struct snd_soc_dapm_widget *w,
	struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_codec * codec = w->codec;

	printk(KERN_INFO "#%s: Widget %s Event %d\n", __func__, w->name, event);
	
	if (event == SND_SOC_DAPM_POST_PMU) {
		/* Unmute the RECP and RECM Volume Control */
		snd_soc_update_bits (codec, REC_AMP_CNTL_R5, 0x3F, 0x0B);
	}	
	if (event == SND_SOC_DAPM_PRE_PMD)
	{
		/* mute the RECP and RECM Volume Control */
		snd_soc_update_bits (codec, REC_AMP_CNTL_R5, 0x3F, 0x39);
	}
	return 0;
}

int aic3262_fm_pop_cntl(int val)
{
	struct snd_soc_codec *codec = aic3262_codec;
	struct aic3262_priv *aic3262 = snd_soc_codec_get_drvdata(codec);
	if(val ==1) {
		aic3262->fm_mode_state = true;
		return 0;
	}
	else {
		aic3262->fm_mode_state = false;
		mdelay(10);
		snd_soc_update_bits(codec, HP_AMP_CNTL_R1, 0xc0, 0x00);
		mdelay(3);
		return 0;
	}
}
...
static const struct snd_soc_dapm_widget aic3262_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA_E("HPR Driver", HP_AMP_CNTL_R1, 0, 0, NULL, 0,
				aic326x_hp_event, SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMU),
	...
	SND_SOC_DAPM_PGA_E("SPKL Driver", SPK_AMP_CNTL_R1, 1, 0, NULL, 0, 
			spk_driver_event, SND_SOC_DAPM_POST_PMU),
	...
	SND_SOC_DAPM_PGA_E("RECP Driver", REC_AMP_CNTL_R5, 7, 0, NULL, 0,
		rec_power_event, SND_SOC_DAPM_POST_PMU | SND_SOC_DAPM_PRE_PMD),
...
/* Capture (ADC) portions */
	...
	#if 0
	{"Left Input PGA", NULL, "LIP"},
	{"Right Input PGA", NULL, "RIP"},
	{"Left ADC", NULL, "Left Input PGA"},
	{"Right ADC", NULL, "Right Input PGA"},
	#endif
/* ASI Output Routing */
...
int aic3262_change_page(struct snd_soc_codec *codec, u8 new_page)
{
	...
	u8 data[2];
	int ret = 0;
	//DBG(KERN_INFO "%s : entered with new page value = %d\n",__func__,new_page);
	data[0] = 0;
	data[1] = new_page;
	...
	if(aic3262->bus_type == SND_SOC_SPI) {
		if (codec->hw_write(codec->control_data, data, 2) <  0) {
			printk(KERN_ERR "#%s: Error in spi write for Page %d\n",
					__func__, new_page);
			return -EIO;
		}
	} else {
		if (codec->hw_write(codec->control_data, data, 2) < 0) {
			printk(KERN_ERR "#%s: Error in i2c write for Page %d\n",
					__func__, new_page);
			return -EIO;
		}
	}
...
int aic3262_change_book(struct snd_soc_codec *codec, u8 new_book)
{
	...
	u8 data[2];
	int ret = 0;
	//DBG(KERN_INFO "%s : entered with new book value = %d\n",__func__,new_book);
	data[0] = 0x7F;
	data[1] = new_book;

	ret = aic3262_change_page(codec, 0);
	if (ret < 0) {
		printk(KERN_ERR "#%s: Error While Changing to Page 0 from %d\n",
			__func__, aic3262->page_no);
		return ret;
	}	
	...
	if(aic3262->bus_type == SND_SOC_SPI) {
		if (codec->hw_write(codec->control_data, data, 2) <  0) {
			printk(KERN_ERR "#%s: Error in spi write for Book %d\n",
					__func__, new_book);
			return -EIO;
		}
	} else {
		if (codec->hw_write(codec->control_data, data, 2) < 0) {
			printk(KERN_ERR "#%s: Error in i2c write for Book %d\n",
					__func__, new_book);
			return -EIO;
		}
	}
...
u8 aic3262_read(struct snd_soc_codec *codec, u16 reg)
{
	...
	if (reg >= AIC3262_CACHEREGNUM) {
		printk(KERN_ERR "#%s: Invalid Register Offset %d\n",
				__func__, reg);
		return 0;
	}
	...
	if (aic3262->page_no != page) {
		status = aic3262_change_page(codec, page);

		if (status < 0 ) {
			mutex_unlock(&codec_io_mutex);			
			printk(KERN_ERR "#%s: Error in Change Page %d\n",
					__func__, page);
			return 0;		
		}
	}
...
int aic3262_write(struct snd_soc_codec *codec, u16 reg, u8 value)
{
	...
	*/
	...
	if(aic3262->bus_type == SND_SOC_SPI) {
		if (codec->hw_write(codec->control_data, data, 2) <  0) {
			mutex_unlock(&codec_io_mutex);
			printk(KERN_ERR "#%s: Error in SPI Write for Pg %d Reg %d\n",
					__func__, page, reg);
			return -EIO;
		}
	} else {
		if (codec->hw_write(codec->control_data, data, 2) !=  2) {
			mutex_unlock(&codec_io_mutex);
			printk(KERN_ERR "#%s: Error in I2C Write for Pg %d Reg %d\n",
					__func__, page, reg);
			return -EIO;
		}
	}
...
int aic3262_i2c_write(struct i2c_client *client, const char *buf, int count)
{
	u8 data[3];
	int ret;
	data[0] = *buf;
...
static int aic3262_spi_write(struct spi_device *spi, u8 *buf,  int count)
{
	...
	spi_cs_en(1);
	if(ret < 0) {
		printk(KERN_ERR "#%s: Error in SPI Write for buffer [0x%x]\n",
					__func__, data[0]);
		return -EIO;
		}
	kzfree(data);
...
#if 0
		val = aic3262_read(codec, aic3262_reg_init[i].reg_offset);
		printk("the pre-value of register %d\t is %d\t and the value being written is %d\n",aic3262_reg_init[i].reg_offset,
								val,aic3262_reg_init[i].reg_val);
#endif	
		ret = snd_soc_write(codec, aic3262_reg_init[i].reg_offset,
			aic3262_reg_init[i].reg_val);
#if 0
		val = aic3262_read(codec, aic3262_reg_init[i].reg_offset);
		printk("the post-value of register %d\t is %d\n",aic3262_reg_init[i].reg_offset,val);
#endif	
		if (ret)
			break;
	}
 ...
 *----------------------------------------------------------------------------
 */
unsigned int aic3262_hw_read(struct snd_soc_codec *codec, u8 data)
{
	u8 buf;
	int rc;
	data = (data << 1) | 0x01;
...
static irqreturn_t aic3262_jack_handler(int irq, void *data)
{
	...
	unsigned int bit4mask,bit5mask;
	DBG(KERN_INFO "%s++\n", __func__);
...
#elif defined  CONFIG_SWITCH_GPIO 

	if(be_headset_state == false){
		pr_err("Not headset state ,return directly.\n");
		return IRQ_HANDLED;
		}
	/* Read the Jack Status Register*/
	...
	value = snd_soc_read(codec, STICKY_FLAG3);
	printk("reg45 0x%x\n", value);

	value = snd_soc_read(codec, DAC_FLAG_R1);
	pr_err("DAC_FLAG_R1  reg37 = 0x%x\n", value);
	
	bit4mask = value & 0x10;
	bit5mask = value & 0x20;
	if((bit4mask | bit5mask)== 0x10)//headphone without MIC
	{
	 	printk("headphone without MIC device , do not report key---headset button\n");
		return IRQ_HANDLED;
	}
	...
	if (value & INT_FLAG2_BUTTN_PRESSBIT)
	{
	     printk("report key---headset button\n");
	     input_report_key(aic3262->button_dev,  KEY_MEDIA, 1);	 
	     input_sync(aic3262->button_dev);

            input_report_key(aic3262->button_dev,  KEY_MEDIA, 0);	
	     input_sync(aic3262->button_dev);
...
static int aic3262_probe(struct snd_soc_codec *codec)
{
	...
	if (aic3262->irq) {
		...
		ret = enable_irq_wake(aic3262->irq);
		if (ret != 0) {
			dev_warn(codec->dev, "Can't enable IRQ as wake source: %d\n",
			 ret);
			}		
	} else {
	...
	/*TODO*/
	snd_soc_write(codec, MIC_BIAS_CNTL, 0x22);
...
void spi_cs_en(unsigned int cs)
{
	if(cs ==1)
	{
		gpio_direction_output(TEGRA_CODEC_SPI_CS, 1);
		udelay(1);
	}
	if(cs == 0)
	{
		gpio_direction_output(TEGRA_CODEC_SPI_CS, 0);
		udelay(1);
	}
}
...
static int __devinit aic3262_spi_probe(struct spi_device *spi)
{
	...
	if(gpio_request(TEGRA_CODEC_SPI_CS, "SPI_CS_SELECT") < 0) {
		printk("Failed to request SPI_CS_SELECT 0x%x\n",TEGRA_CODEC_SPI_CS);
		return -1;
	} else {
		tegra_gpio_enable(TEGRA_CODEC_SPI_CS);
	}
...
static int __init tlv320aic3262_modinit(void)
{
	...
	printk("calling i2c add driver\n");
	ret = i2c_add_driver(&tlv320aic3262_i2c_driver);
		printk(KERN_INFO "I2c driver registering\n");
		if (ret != 0) {
			printk(KERN_ERR
				"Failed to register TLV320AIC3362 I2C driver: %d\n",
				ret);
			}
 	} else if(versionid==6||versionid==5||versionid==4){
		printk(KERN_INFO "spi driver registering\n");
	ret = spi_register_driver(&aic3262_spi_driver);
	if (ret != 0)
		printk(KERN_ERR "Failed to register aic3262 SPI driver: %d\n", ret);
 	}
	else{
		printk(KERN_INFO "Not right board version ,and register SPI to try\n");
	ret = spi_register_driver(&aic3262_spi_driver);
	if (ret != 0)
		printk(KERN_ERR "Failed to register aic3262 SPI driver: %d\n", ret);	
 	}
...
static void __exit tlv320aic3262_exit(void)
{
	...
	if(versionid==0||versionid==1){
			...
			printk(KERN_INFO "#%s:i2c Exited\n", __func__);
		}else if(versionid==6||versionid==5||versionid==4){
			spi_unregister_driver(&aic3262_spi_driver);
			printk(KERN_INFO "#%s:spi Exited\n", __func__);
		}
		else{
			spi_unregister_driver(&aic3262_spi_driver);
			printk(KERN_INFO "#%s:spi Exited for not right board version\n", __func__);
		}
}
...
MODULE_LICENSE("GPL");
MODULE_VERSION("TI TLV320AIC3262 R01V01B02 16062012");

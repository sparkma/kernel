...
//[ECID:000000] ZTEBSP jiaobaocun 20120110 start, for headset 
#include <trace/events/asoc.h>
#include <linux/wakelock.h>
#include "../../../arch/arm/mach-tegra/gpio-names.h"
extern bool be_headset_state;
extern int headset_type;
extern void headphone_poll_timer_func(void);
struct wake_lock jack_wake_dealy;
//[ECID:000000] ZTEBSP jiaobaocun 20120110 start, for headset 
/**
...
{
	int i;
	wake_lock_init(&jack_wake_dealy, WAKE_LOCK_SUSPEND, "jack_dealy");
	for (i = 0; i < count; i++) {
		if (!pins[i].pin) {
...
static void snd_soc_jack_gpio_detect(struct snd_soc_jack_gpio *gpio)
{
	...
	if(report)		//headset plug in
	{
	  gpio_set_value(TEGRA_GPIO_PV1, 0);
	  mdelay(5);
        gpio_set_value(TEGRA_GPIO_PV1, 1);
	 pr_err("TEGRA_GPIO_PV1 has set to high!\n");
	 }
	else //headset plug out
	{
		 gpio_set_value(TEGRA_GPIO_PV1, 0);
		 pr_err("TEGRA_GPIO_PV1 has set to low!\n");
		 be_headset_state = false;
		 pr_err("be_headset_state = false;\n");  
	}
	snd_soc_jack_report(jack, report, gpio->report);
	pr_err("headset jack reported!\n");
	wake_lock_timeout(&jack_wake_dealy,10*HZ);
	if(report)	{
	headphone_poll_timer_func();}
	#if 0
	if (gpio->jack_status_check)
		report = gpio->jack_status_check();
	...
	#endif
	//[ECID:000000] ZTEBSP jiaobaocun 20120110 end, for headset 
}

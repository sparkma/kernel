...
#include <mach/legacy_irq.h>	//[ECID:000000] ZTEBSP wanghaifei 20120326, add nvidia patch for bug 958511
...
//[ECID:0000] ZTEBSP dingzhihui 20120307 change for bt sleep power up,start 

#define BLUETOOTH_POWER_GPIO_OFFSET 0x224
#define BLUETOOTH_POWER_EN_MASK 0x8

static bool is_specdev_enable()
{
    bool ret;
    u32 val;

    // is bluetooth enabled
   val = readb(IO_TO_VIRT(TEGRA_GPIO_BASE + BLUETOOTH_POWER_GPIO_OFFSET));
   ret |= ((val & BLUETOOTH_POWER_EN_MASK) != 0);

    return ret;
}
extern int zte_get_hw_board_id(void);
//[ECID:0000] ZTEBSP dingzhihui 20120307 change for bt sleep power up,end 
static int tegra_suspend_enter(suspend_state_t state)
{
	int ret;
	ktime_t delta;
	struct timespec ts_entry, ts_exit;
//[ECID:0000] ZTEBSP dingzhihui 20120307 change for bt sleep power,start 
   /* if special device is enabled, entering lp1 mode, not lp0 and lp2 */

   int hw_id=0;
   hw_id=zte_get_hw_board_id();
   if((0x01==hw_id)||(0x03==hw_id))
   	{
           if (is_specdev_enable()) 
	      {
             current_suspend_mode=TEGRA_SUSPEND_LP1;
          }
   	}
   //[ECID:0000] ZTEBSP dingzhihui 20120306 modify for  new board ,end 
...
//[ECID:000000] ZTEBSP wanghaifei start 20120326, add nvidia patch for BUG 958511
	else if (mode == TEGRA_SUSPEND_LP1) {
		*iram_cpu_lp1_mask = 1;
		tegra_legacy_irq_set_lp1_wake_mask();
	}
//[ECID:000000] ZTEBSP wanghaifei end 20120326, add nvidia patch for BUG 958511
...
#endif
//[ECID:000000] ZTEBSP wangxiaomei start 20111021 
void NvBlForceRecoveryMode(void)
{
        u32 regval = readl(pmc + PMC_SCRATCH0);
       pmc_32kwritel(regval | 2, PMC_SCRATCH0);
}
//[ECID:000000] ZTEBSP wangxiaomei end 20111021 

...
static __initdata struct tegra_pingroup_config whistler_null_ulpi_pinmux[] = {
	...
	{TEGRA_PINGROUP_UAC, TEGRA_MUX_RSVD4, TEGRA_PUPD_NORMAL,
	 TEGRA_TRI_NORMAL},
//[ECID:000000] ZTEBSP maoke 20111012 start, for AP+Modem ULPI
	{TEGRA_PINGROUP_GPU, TEGRA_MUX_UARTA, TEGRA_PUPD_PULL_UP,
	 TEGRA_TRI_NORMAL},
//[ECID:000000] ZTEBSP maoke 20111012 end, for AP+Modem ULPI
	 
};

static struct tegra_ulpi_trimmer e1219_trimmer = { 11, 1, 1, 1 };//zte_modify, nv patch for ulpi stablity
...
struct platform_device *ulpi_usb_pdev = NULL;

static struct platform_device *
tegra_null_ulpi_usb_host_register(struct platform_device *ehci_device,
...
static void tegra_null_ulpi_usb_host_unregister(struct platform_device *pdev)
{
...
void tegra_start_usb_host()
{
	if (!ulpi_usb_pdev) {
		ulpi_usb_pdev = tegra_null_ulpi_usb_host_register(&tegra_ehci2_device,
							&ehci2_null_ulpi_platform_data);
	}
}

void tegra_stop_usb_host()
{
	if (ulpi_usb_pdev) {
		tegra_null_ulpi_usb_host_unregister(ulpi_usb_pdev);
		ulpi_usb_pdev = NULL;
	}
}

...
static int tegra_emc_read_mrr(unsigned long addr, u32 dev)
{
	u32 value;
	int count = 100;
	...
	if (count == 0) {
        ...
        if(0==dev)
	    value = (0x1 << 31) | (addr << 16);
        else if(1==dev)
            value = (0x1 << 30) | (addr << 16);
...
#define EMC_TABLE_SS1G_ONLY    0        //1 -- samsung 1G(2die) memory
                                        //0 -- samsung 1G(2die) and samsung 512M(1die) memory
void tegra_init_emc(const struct tegra_emc_chip *chips, int chips_size)
{
	...
	int pid;
	int chip_matched = -1;
        int chip_matches[2]={-1,-1};
        int m=0;

	vid = tegra_emc_read_mrr(5,0);
	rev_id1 = tegra_emc_read_mrr(6,0);
	rev_id2 = tegra_emc_read_mrr(7,0);
	pid = tegra_emc_read_mrr(8,0);
        vid1 = tegra_emc_read_mrr(5,1);

	for (i = 0; i < chips_size; i++) {
#if EMC_TABLE_SS1G_ONLY
		...
		break;
#else 
                if(chips[i].mem_manufacturer_id == vid && chips[i].mem_revision_id1==rev_id1
                                                       && chips[i].mem_revision_id2==rev_id2 &&  chips[i].mem_pid==pid)
                        chip_matches[m++] = i;
                else 
                        continue;
#endif
...
#if (!EMC_TABLE_SS1G_ONLY)
                if(vid!=vid1)
                        chip_matched=chip_matches[0];
                else if(vid==vid1)
                        chip_matched=chip_matches[1];
#endif
	if (chip_matched >= 0) {
		pr_info("%s: %s memory found\n", __func__,
			chips[chip_matched].description);
                pr_info("%s: Memory vid     = 0x%04x", __func__, vid);
                pr_info("%s: Memory vid1    = 0x%04x", __func__, vid1);

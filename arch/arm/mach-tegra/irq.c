...
void tegra_legacy_irq_set_lp1_wake_mask(void)
{
		...
		//printk("lp1 original mask [%d]0x%x\n", i, tegra_legacy_saved_mask[i]);
		//printk("lp1 new mask [%d]0x%x\n", i, tegra_legacy_wake_mask[i]);
...
#ifdef CONFIG_PM_SLEEP
//[ECID:000000] ZTEBSP wanghaifei start 20120326, add nvidia patch for BUG 958511
static int tegra_set_lp1_wake(int irq, int enable)
{
        //printk("tegra_set_lp1_wake for irq %d ========= \n", irq);
...
//[ECID:000000] ZTEBSP wanghaifei start 20120326, add nvidia patch for BUG 958511
	memset(tegra_legacy_wake_mask, 0, sizeof(tegra_legacy_wake_mask));
	memset(tegra_legacy_saved_mask, 0, sizeof(tegra_legacy_saved_mask));
//[ECID:000000] ZTEBSP wanghaifei end 20120326, add nvidia patch for BUG 958511
}

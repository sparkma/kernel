...
#define APB_MISC_GP_UAACFGPADCTRL_0	0x8ac
#define APB_MISC_GP_UABCFGPADCTRL_0	0x8b0
#define APB_MISC_GP_UADCFGPADCTRL_0	0x90c
...
void null_phy_set_cfgpadctrl(void)
{
	...
	/* set HSM_EN */
	val = readl(apb_misc + APB_MISC_GP_UAACFGPADCTRL_0);
	val |= (1<<2);
	writel(val, apb_misc + APB_MISC_GP_UAACFGPADCTRL_0);

	val = readl(apb_misc + APB_MISC_GP_UABCFGPADCTRL_0);
	val |= (1<<2);
	writel(val, apb_misc + APB_MISC_GP_UABCFGPADCTRL_0);

	val = readl(apb_misc + APB_MISC_GP_UADCFGPADCTRL_0);
	val |= (1<<2);
	writel(val, apb_misc + APB_MISC_GP_UADCFGPADCTRL_0);
}
...
static int null_phy_power_on(struct tegra_usb_phy *phy, bool is_dpd)
{
	...
	writel(val, base + USB_SUSP_CTRL);
#endif
	if (!phy->enable_ulpi_pad_clk) {
		val = readl(base + ULPI_TIMING_CTRL_0);
		val |= ULPI_CLK_PADOUT_ENA;
		writel(val, base + ULPI_TIMING_CTRL_0);
		phy->enable_ulpi_pad_clk = true;
	}
...
//zte_modify nv patch begin
//extern int tegra_baseband_phy_off(void);
//zte_modify nv patch end
...
//zte_modify nv patch begin
#if 0
	else if (phy->usb_phy_type == TEGRA_USB_PHY_TYPE_NULL_ULPI) {
		tegra_baseband_phy_off();
	}
#endif
//zte_modify nv patch end
	if (phy->mode == TEGRA_USB_PHY_MODE_HOST) {
